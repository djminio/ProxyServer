...
//		_asm int 3;
	...
	for( pToConnectServer = this->m_pServerListHead; pToConnectServer; pToConnectServer = pToConnectServer->pNextServerData )
	{
		...
		{
			g_pRMTable->m_ListenerTable.Add( pToConnectServer->szIP , 0);	//접속 안한상태는 두번째 인자값 0으로 Add
...
void CServerTable::ShowListenerStatus()
{
	...
	while (cur)
	{
		...
		MyLog( LOG_JUST_DISPLAY, "[%2d] SERVER_TYPE_RMLISTENER / %15s / %5d / (Connected)"      //Ｘ○
