...
//			MyLog( LOG_IMPORTANT, "WARNING : No Agent server are ready to service" );
	...
	else
	{
       // 접속한 유저에게 Agent의 IP를 보내준다. 
		...
		buffer.size = 18;
		memcpy(msg,&buffer,5);
		memcpy(msg+5,pToAssign->szIPForUser,16);
		...
		MyLog( LOG_NORMAL, "OK : User(%d) push to %s:%d", dwConnectionIndex, pToAssign->szIPForUser, pToAssign->wPortForUser);
	...
	case PTCL_SERVERSET_USERNUM_REQUEST:
		{
			...
			unsigned short count = static_cast<WORD>((g_pServerTable->m_dwNumOfUsersInServerSet));
...
{
	// Added by chan78 at 2001/03/16
// If the connected client sends a packet, if it is not a CONTROL_CLIENT authentication attempt packet
// Assume that this is an attempt to harm the server, leave the necessary records and then disconnect.
		...
		case RM_TYPE_LISTENER:
			{
				...
				{
					char ip[IP_LENGTH];	memset(ip,0,IP_LENGTH);
					WORD port;
